...
print("=" * 80)
print()
print(f"{'Metric':<30} {'Run 1':>10} {'Run 2':>10} {'Run 3':>10} {'Δ1→2':>8} {'Δ2→3':>8}")
print("-" * 80)
...
for label, key in metrics:
    ...
    print(f"{label:<30} {val1:>9.1%} {val2:>9.1%} {val3:>9.1%} {diff1_2*100:>+7.1f}pp {diff2_3*100:>+7.1f}pp")
...
em_recovery = (run3['metrics']['answer_exact_match'] - run1['metrics']['answer_exact_match']) * 100
f1_recovery = (run3['metrics']['answer_f1'] - run1['metrics']['answer_f1']) * 100
recall_recovery = (run3['metrics']['retrieval_recall'] - run1['metrics']['retrieval_recall']) * 100

print("✅ REVERT SUCCESSFUL - Performance Restored:")
print()
print(f"  Answer Exact Match: {run2['metrics']['answer_exact_match']:.1%} → {run3['metrics']['answer_exact_match']:.1%}")
print(f"    Recovered from regression (33% → 36%)")
print()
print(f"  Answer F1: {run2['metrics']['answer_f1']:.1%} → {run3['metrics']['answer_f1']:.1%}")
print(f"    Improved by +2.6pp over Run 2")
print()
print(f"  Retrieval Recall: {run2['metrics']['retrieval_recall']:.1%} → {run3['metrics']['retrieval_recall']:.1%}")
...
if abs(em_recovery) < 1:
    print(f"✓ Exact Match: {run1['metrics']['answer_exact_match']:.1%} → {run3['metrics']['answer_exact_match']:.1%} ({em_recovery:+.1f}pp)")
    print("  Performance matches baseline")
else:
    print(f"  Exact Match: {run1['metrics']['answer_exact_match']:.1%} → {run3['metrics']['answer_exact_match']:.1%} ({em_recovery:+.1f}pp)")

if f1_recovery > 0:
    print(f"✓ Answer F1: {run1['metrics']['answer_f1']:.1%} → {run3['metrics']['answer_f1']:.1%} ({f1_recovery:+.1f}pp)")
    print("  Slight improvement over baseline!")
else:
    print(f"  Answer F1: {run1['metrics']['answer_f1']:.1%} → {run3['metrics']['answer_f1']:.1%} ({f1_recovery:+.1f}pp)")

if abs(recall_recovery) < 1:
    print(f"≈ Retrieval Recall: {run1['metrics']['retrieval_recall']:.1%} → {run3['metrics']['retrieval_recall']:.1%} ({recall_recovery:+.1f}pp)")
...
print("=" * 80)
print()
print("Reverting the aggressive type filtering (0.2 threshold + hard filter) successfully:")
print()
