...
print(f"  Exact Match:    {baseline['answer_exact_match']:.2%} → {new['metrics']['answer_exact_match']:.2%}  ({(new['metrics']['answer_exact_match'] - baseline['answer_exact_match']) * 100:+.1f}pp)")
print(f"  F1 Score:       {baseline['answer_f1']:.2%} → {new['metrics']['answer_f1']:.2%}  ({(new['metrics']['answer_f1'] - baseline['answer_f1']) * 100:+.1f}pp)")
print(f"  Fuzzy Match:    N/A → {new['metrics']['answer_fuzzy_match']:.2%}")
print()

print("RETRIEVAL QUALITY:")
print(f"  Precision:      {baseline['retrieval_precision']:.2%} → {new['metrics']['retrieval_precision']:.2%}  ({(new['metrics']['retrieval_precision'] - baseline['retrieval_precision']) * 100:+.1f}pp)")
print(f"  Recall:         {baseline['retrieval_recall']:.2%} → {new['metrics']['retrieval_recall']:.2%}  ({(new['metrics']['retrieval_recall'] - baseline['retrieval_recall']) * 100:+.1f}pp)")
print(f"  F1 Score:       {baseline['retrieval_f1']:.2%} → {new['metrics']['retrieval_f1']:.2%}  ({(new['metrics']['retrieval_f1'] - baseline['retrieval_f1']) * 100:+.1f}pp)")
print()
