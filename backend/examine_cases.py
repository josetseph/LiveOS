...
print(
    f"  Exact: {exact_count} / {len(results['results'])} ({exact_count/len(results['results']):.1%})"
)
print(
    f"  Fuzzy: {fuzzy_count} / {len(results['results'])} ({fuzzy_count/len(results['results']):.1%})"
)
...
print(
    f"  Medium (0.10-0.20): {sum(1 for p in retrieval_precisions if 0.10 <= p <= 0.20)} cases"
)
...
if high_precision:
    ...
    print(f"\nExact Match Rate:")
    print(
        f"  High retrieval precision (>0.20): {high_em:.1%} ({len(high_precision)} cases)"
    )

if low_precision:
    low_em = sum(1 for r in low_precision if r["exact_match"]) / len(low_precision)
    print(
        f"  Low retrieval precision (<0.10): {low_em:.1%} ({len(low_precision)} cases)"
    )
