...
if __name__ == "__main__":
    ...
    with graph_service.driver.session() as session:
        # Note ID Uniqueness
        session.run("CREATE CONSTRAINT note_id_unique IF NOT EXISTS FOR (n:Note) REQUIRE n.id IS UNIQUE")
        ...
        try:
             # Create vector index for 4096 dimensions (Qwen/Ollama)
             session.run("""
        ...
        try:
            async with await s3_client() as client:
                ...
                if 'Contents' in response:
                    ...
                    await client.delete_objects(Bucket=bucket_name, Delete={'Objects': delete_keys})
