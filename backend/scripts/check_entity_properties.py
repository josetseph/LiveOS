...
def main():
    ...
    for entity in result2:
        print(f"{entity['name']:20} | type={entity.get('type'):15} | entity_type={entity.get('entity_type'):15} | summary_len={entity.get('summary_len')}")
    ...
    count_queries = [
        ("Has 'type' property", "MATCH (e:Entity) WHERE e.type IS NOT NULL RETURN count(e) as cnt"),
        ("Has 'entity_type' property", "MATCH (e:Entity) WHERE e.entity_type IS NOT NULL RETURN count(e) as cnt"),
        ("Has neither", "MATCH (e:Entity) WHERE e.type IS NULL AND e.entity_type IS NULL RETURN count(e) as cnt"),
    ]
