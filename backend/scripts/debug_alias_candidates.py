...
async def main():
    ...
    print(
        "5️⃣  Checking entities meeting ALL criteria (candidates for alias detection)..."
    )
    result = graph_service.execute_query(
        """
        MATCH (e:Entity)
    ...
    if result:
        for i, entity in enumerate(result, 1):
            ...
            print(
                f"       Summary length: {summary_len} | Has alias link: {alias_status}"
            )
        print()
    ...
    if candidates == 0 and total > 0:
        if long_summary == 0:
            print(
                "• No entities have summaries. Run ingestion to populate entity summaries."
            )
        ...
        print(
            f"   Run: python scripts/detect_aliases.py --limit {min(candidates, 100)}"
        )
    else:
