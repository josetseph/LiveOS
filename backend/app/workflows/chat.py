...
class ChatWorkflow:
    async def chat(self, user_query: str) -> dict:
        ...
        print(f"[Chat] Retrieval took: {t1 - t0:.4f}s (Docs found: {len(top_docs)})")
        ...
        answer = ""
        if not context:
            answer = "I couldn't find any relevant context in your brain to answer that."
            print("[Chat] No context found. Skipping generation.")
        else:
            t2 = time.perf_counter()
            # Pass only text context to LLM
            answer = await llm_service.synthesize(context, user_query)
            t3 = time.perf_counter()
            print(f"[Chat] Generation took: {t3 - t2:.4f}s")
        ...
        print(f"[Chat] Total pipeline duration: {time.perf_counter() - start_time:.4f}s\n")
        return {
            "query": user_query,
            "answer": answer,
