"""
Hop-Pipeline v4 (Gemini) — Google Gemini-backed evaluator.
...
Usage:
    python tests/benchmark/evaluate_hop_pipeline_v4_gemini.py
    python tests/benchmark/evaluate_hop_pipeline_v4_gemini.py --limit 20 --echo
"""
...
# Gemini calls use the native async client (.aio) with a single persistent
...
async def _gemini_rewrite_back_refs(sub_questions: list[str]) -> list[str]:
    """Async version of _rewrite_back_references."""
...
async def main():
    parser = argparse.ArgumentParser(
        description="Hop-Pipeline v4 (Gemini): sequential Gemini-backed evaluation"
    )
    parser.add_argument(
    ...
    log_path = results_dir / f"hop_v4_gemini_{args.dataset}{limit_tag}_{timestamp}.txt"
    json_path = (
        results_dir / f"hop_v4_gemini_{args.dataset}{limit_tag}_{timestamp}.json"
    )

    L = Logger(log_path, echo=args.echo)
    L.log(f"Hop-Pipeline v4 (Gemini) — {args.dataset.upper()}")
