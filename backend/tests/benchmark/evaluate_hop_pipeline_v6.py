"""
Hop-Pipeline v6 — full HotPotQA evaluation with open-ended back-reference rewriting.

Changes from v4:
  - Back-reference rewriting is now always attempted for every sub-question after
    the first, with the LLM deciding whether rewriting is needed rather than a
    hard-coded regex.  This catches arbitrary placeholders ('that car', 'that
    building', 'the company', etc.) that the old regex would silently skip.
  - All other logic (SYNTHESIS_RULES, fuzzy-pass F1 threshold, answer-type
    constraint) is identical to v4.

Usage:
    python tests/benchmark/evaluate_hop_pipeline_v6.py
    python tests/benchmark/evaluate_hop_pipeline_v6.py --limit 20 --echo
"""
...
async def main():
    parser = argparse.ArgumentParser(
        description="Hop-Pipeline v6: open-ended back-reference rewriting (LLM decides)"
    )
    parser.add_argument(
    ...
    log_path = results_dir / f"hop_v6_{args.dataset}{limit_tag}_{timestamp}.txt"
    json_path = results_dir / f"hop_v6_{args.dataset}{limit_tag}_{timestamp}.json"

    L = Logger(log_path, echo=args.echo)
    L.log(f"Hop-Pipeline v6 — {args.dataset.upper()}")
