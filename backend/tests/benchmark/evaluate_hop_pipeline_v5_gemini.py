"""
Hop-Pipeline v5 (Gemini) — Google Gemini-backed evaluator.
...
Usage:
    python tests/benchmark/evaluate_hop_pipeline_v5_gemini.py
    python tests/benchmark/evaluate_hop_pipeline_v5_gemini.py --limit 20 --echo
"""
...
async def main():
    parser = argparse.ArgumentParser(
        description="Hop-Pipeline v5 (Gemini): sequential Gemini-backed evaluation"
    )
    parser.add_argument(
    ...
    log_path = results_dir / f"hop_v5_gemini_{args.dataset}{limit_tag}_{timestamp}.txt"
    json_path = (
        results_dir / f"hop_v5_gemini_{args.dataset}{limit_tag}_{timestamp}.json"
    )

    L = Logger(log_path, echo=args.echo)
    L.log(f"Hop-Pipeline v5 (Gemini) — {args.dataset.upper()}")
